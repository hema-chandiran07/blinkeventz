...
@Controller('venues/reviews')
export class VenueReviewsController {
  constructor(private readonly prisma: PrismaService) {}

  /**
   * Get all reviews for the current venue owner's venues
   */
  @Get('me')
  @ApiOperation({ summary: 'Get all reviews for current venue owner venues' })
  ...
  async getMyReviews(
    ...
    return {
      reviews: reviews.map(r => ({
        id: r.id,
        venueName: r.venue?.name || 'Unknown Venue',
  ...
  async replyToReview(
    ...
    // Verify the review belongs to one of the user's venues
    const venues = await this.prisma.venue.findMany({
      where: { ownerId: userId },
      select: { id: true },
    });
    ...
    if (venues.length === 0) {
      throw new NotFoundException('No venues found for this user');
    }
    ...
    if (!review) {
      throw new NotFoundException('Review not found or does not belong to your venues');
    }
